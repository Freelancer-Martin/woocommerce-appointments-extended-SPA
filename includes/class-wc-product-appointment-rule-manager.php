...
class WC_Product_Appointment_Rule_Manager {
	...
	public static function check_availability_rules_against_time( $appointable_product, $slot_start_time, $slot_end_time, $staff_id, $get_capacity = false, $appointable = null ) {
		...
		foreach ( $rules as $rule ) {
			...
			#print '<pre>'; print_r( $rule['qty'] ); print '</pre>';
			...
			*/
			...
			if ( in_array( $type, array( 'days', 'custom', 'months', 'weeks' ) ) && ( $rule_end_time <= $rule_start_time ) ) {
				...
				if ( $slot_start_time >= $rule_start_time && $slot_end_time <= $rule_end_time ) {
					...
					#print '<pre>'; print_r( 'x11' . '__' . date( 'ymd H:i', $rule_start_time ) .'__'.date( 'ymd H:i', $rule_end_time ) .'_||_'. date( 'ymd H:i', $slot_start_time ) .'__'.date( 'ymd H:i', $slot_end_time ) .'=='. $rule_val ); print '</pre>';
					$appointable = $rule_val;
					$capacity = $qty;
					continue;
				}
				...
				if ( 'hour' === $appointable_product->get_duration_unit()
					 && $slot_start_time > $rule_start_time
	 				 ...
	 				 && $slot_end_time < $rule_end_time ) {
						 ...
						 #print '<pre>'; print_r( 'x10' . '__' . date( 'ymd H:i', $rule_start_time ) .'__'.date( 'ymd H:i', $rule_end_time ) .'_||_'. date( 'ymd H:i', $slot_start_time ) .'__'.date( 'ymd H:i', $slot_end_time ) .'=='. $rule_val ); print '</pre>';
				...
				if ( ! $rule_val
					 ...
					 && $slot_start_time < $rule_end_time ) {
						 ...
						 #print '<pre>'; print_r( 'x09' . '__' . date( 'ymd H:i', $rule_start_time ) .'__'.date( 'ymd H:i', $rule_end_time ) .'_||_'. date( 'ymd H:i', $slot_start_time ) .'__'.date( 'ymd H:i', $slot_end_time ) .'=='. $rule_val ); print '</pre>';
				...
				if ( ! $rule_val
					 ...
					 && $slot_end_time <= $rule_end_time ) {
						 ...
						 #print '<pre>'; print_r( 'x08' . '__' . date( 'Y-m-d H:i', $slot_start_time ) . '__' . date( 'Y-m-d H:i', $slot_end_time ) . ' ruleval=' .$rule_val ); print '</pre>';
