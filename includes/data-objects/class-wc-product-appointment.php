...
class WC_Product_Appointment extends WC_Product_Appointment_Compatibility {
	...
	public function __construct( $product = 0 ) {
		...
		//$this->$url_parameteters = parse_url(wp_get_referer());

		// Frontend: custom select field in product single pages
		add_action( 'woocommerce_before_add_to_cart_button', array( $this, 'fabric_length_product_field' ) );
		// Add selected pack data as custom data to cart items
		//add_filter( 'woocommerce_add_cart_item_data', array( $this, 'add_pack_data_to_cart_item_data', 20, 2 ) );
...
*/
	...
	*/
					...
					if ( $v['from'] <= strtotime( date( 'G:i', $display_slot ) ) && $v['to'] > strtotime( date( 'G:i', $display_slot ) ) ) {
						...
						if ( $quantity_available > 0 ) {
							if ( $quantity['scheduled'] ) {
								...
								$slot_array[] = date_i18n( wc_time_format(), $display_slot );
							} else {
	...
	public function get_product_category( )
	{
		 ...
		 $get_category_ids = $this->get_category_ids();
		 foreach ( $get_category_ids as $id_key => $id_value )
		 {

			 $term = get_term_by( 'id', $id_value, 'product_cat' );
	...
	public function fabric_length_product_field( )
	{
			global $product;

			if( $product->is_type('variable') ) return; // Not variable products

			if ( in_array( "SPA 21", $this->get_product_category() )  || in_array( "Elamus SPA", $this->get_product_category() ) )
			{
			...
			$options = array(
					""          => __('Vali Pilet'),
					"hooldus" => "Soovin ainult hoolitsust",
					"elamus" => "Elamus SPA ligipääs",
					"spa21" => "SPA 21+ ligipääs",
			...
			woocommerce_form_field('cards_pack', array(
					'type'          => 'select',
					'class'         => array('elamus-spa-extra-checkout-field form-row-wide'),
					'label'         => __('Vali sissepääsupilet * ', $domain),
					'required'      => true,
					'options'       => $options,
			),'');
			...
			$prices = array(
					''      => wc_price($base_price),
					'00.00' => wc_price($base_price + $price ),
					'100.00' => wc_price($base_price + 100),
					'200.00' => wc_price($base_price + 200),
			...
			jQuery(function($){
					...
					$("#cards_pack").click(function()
					{
						...
						var filter_date = today.toLocaleDateString("en-US", options);
						var get_product_category = <?php echo json_encode( $this->get_product_category() );   ?>;
						var options = <?php echo json_encode( $options );   ?>;
						var selected_option = $('#cards_pack').children("option:selected").val();
						console.log( get_product_category.includes("SPA 21") );
						...
						switch (filter_date)
						{
							...
							case "Monday":
								...
								array = [];
								var base_price = <?php echo json_encode( $base_price = (float) wc_get_price_to_display( $product ) ); ?>;
								var el1 = "€" + base_price + ".00";
								var el2 = "€" + base_price + ".00";

								b = 'p.price';
								e = '.woocommerce-Price-amount.amount',
								c = 'select[name="cards_pack"]';



								if ( get_product_category.includes("SPA 21") || get_product_category.includes("Elamus SPA") )
								{

									if( selected_option == "spa21" )
									{

										if ( 1159 < time || time > 1500 )
										{
											var base_price = <?php echo json_encode( $base_price = (float) wc_get_price_to_display( $product ) ); ?>;
											var add_price = 25;
											var z = base_price + add_price;
											el = $( ".woocommerce-Price-amount.amount" ).html( "€" + z + ".00" );
											el2 = $( "#hidden_price" ).attr( "value", add_price );
											el3 = $( "#hidden_ticket" ).attr( "value", "SPA21" );//hidden_ticket

										}


										if ( 1500 < time || time > 2300 )
										{
											var base_price = <?php echo json_encode( $base_price = (float) wc_get_price_to_display( $product ) ); ?>;
											var add_price = 29;
											var z = base_price + add_price;
									...
									{
										  var base_price = <?php echo json_encode( $base_price = (float) wc_get_price_to_display( $product ) ); ?>;
											el = $( ".woocommerce-Price-amount.amount" ).html( "€" + base_price + ".00" );
											el3 = $( "#hidden_ticket" ).attr( "value", "hooldus" );//hidden_ticket
									}

									if( selected_option == "elamus" )
									{

										if ( 0959 < time || time > 1500 )
										{
											 var base_price = <?php echo json_encode( $base_price = (float) wc_get_price_to_display( $product ) ); ?>;
											 var add_price = 12;
											 var z = base_price + add_price;
											 el = $( ".woocommerce-Price-amount.amount" ).html( "€" + z + ".00" );
											 el2 = $( "#hidden_price" ).attr( "value", add_price );
											 el3 = $( "#hidden_ticket" ).attr( "value", "Elamus" );//hidden_ticket
...
*/
							...
							case "Saturday":
											...
											//console.log( selected_option == "spa21" );
											if ( get_product_category.includes("SPA 21") || get_product_category.includes("Elamus SPA") )
											{
...
function add_pack_data_to_cart_item_data( $cart_item_data, $product_id )
{
    ...
    $pack_price =  sanitize_text_field( $_POST['hidden_price'] );
		$hidden_ticket =  sanitize_text_field( $_POST['hidden_ticket'] );
		if ( isset( $hidden_ticket ) and ! empty( $hidden_ticket ) )
		{

			$product    = wc_get_product($product_id); // The WC_Product Object
			$base_price = (float) $product->get_price();
			$terms = get_the_terms( $product_id, 'product_cat' );
			$get_categories = strip_tags( $product->get_categories() );

			//var_dump( $hidden_ticket );


			if ( "SPA21" == $hidden_ticket )
			{
				$cards = "SPA 21 €".$pack_price;
			}
			elseif( "Elamus" == $hidden_ticket )
			{
				$cards = "Elamus SPA €".$pack_price;
			}
			elseif( "hooldus" == $hidden_ticket )
			{
				$cards = "Soovin ainult hoolitsust";
			}
			...
			$new_price = /*$base_price +*/ $base_price + $pack_price;
...
function display_custom_cart_item_data( $cart_data, $cart_item ) {
    ...
    if ( isset( $cart_item['pack_data']['new_price'] ) ){
        $cart_data[] = array('name' => __( 'Pilet', $domain ),
